...
class StatusPanel(QWidget):
    ...
    def __init__(self, config, parent: QWidget | None = None):
        ...
        self._cfg = config
        self._running = False
        self._workflow: QThread | None = None
        ...
        self._promo_banner = QWidget()
        self._promo_banner.setObjectName("PromoBanner")
        self._promo_banner.setStyleSheet(
            "QWidget#PromoBanner {"
            "background:#fff7d6;"
            "border:1px solid #f59e0b;"
            "border-radius:10px;"
            "}"
            "QWidget#PromoBanner QLabel#PromoTitle {color:#92400e; font-weight:800; font-size:13px;}"
            "QWidget#PromoBanner QLabel#PromoBody {color:#7c2d12; font-weight:700; font-size:12px;}"
            "QWidget#PromoBanner QLabel#PromoHead {color:#b45309; font-weight:900; font-size:12px;}"
        )
        promo_layout = QHBoxLayout(self._promo_banner)
        promo_layout.setContentsMargins(10, 8, 10, 8)
        promo_layout.setSpacing(10)

        promo_head = QLabel("THÔNG BÁO")
        promo_head.setObjectName("PromoHead")
        promo_layout.addWidget(promo_head, 0, Qt.AlignmentFlag.AlignTop)

        promo_text_wrap = QWidget()
        promo_text_layout = QVBoxLayout(promo_text_wrap)
        promo_text_layout.setContentsMargins(0, 0, 0, 0)
        promo_text_layout.setSpacing(2)

        promo_title = QLabel("Admin có bán tài khoản super grok,veo3 giá rẻ để mọi người sử dụng nhé")
        promo_title.setObjectName("PromoTitle")
        promo_title.setWordWrap(True)

        promo_body = QLabel("Liên hệ ngay Zalo :0384220688")
        promo_body.setObjectName("PromoBody")
        promo_body.setWordWrap(True)

        promo_text_layout.addWidget(promo_title)
        promo_text_layout.addWidget(promo_body)
        promo_layout.addWidget(promo_text_wrap, 1)


        layout.addWidget(self._promo_banner)
    ...
    def _update_empty_state(self) -> None:
        ...
        try:
            ...
            show_log = bool(self.table.rowCount() > 0 or self.isRunning() or has_log)
    ...
    def enqueue_text_to_video(self, prompts: list[str]) -> dict | None:
        QMessageBox.information(self, "GROK only", "Chế độ VEO đã bị loại bỏ. Vui lòng dùng tab GROK.")
        return None
    ...
    def enqueue_generate_image_from_prompts(self, items: list[dict]) -> dict | None:
        QMessageBox.information(self, "GROK only", "Chế độ VEO đã bị loại bỏ.")
        return None
    ...
    def start_queued_job(self, mode_key: str, rows: list[int]) -> bool:
        ...
        if normalized_mode not in {self.MODE_GROK_TEXT_TO_VIDEO, self.MODE_GROK_IMAGE_TO_VIDEO}:
            self._append_run_log(f"⚠️ Bỏ qua mode không hỗ trợ: {normalized_mode}")
            for r in clean_rows:
                self._set_row_status_detail(int(r), "FAILED", self._format_failed_status_text("MODE_REMOVED", "Đã loại bỏ chế độ VEO"))
            return False

        self._retry_mode_queue = []
        started = self._start_mode_group(normalized_mode, clean_rows)
        if started:
    ...
    def _add_row(self, row: int, prompt: str) -> None:
        ...
        mode_item = QTableWidgetItem(self._mode_label(self.MODE_GROK_TEXT_TO_VIDEO))
    ...
    def _on_thumbnail_ready(self, src_video_path: str, thumb_path: str) -> None:
        ...
        for row in range(self.table.rowCount()):
            ...
            # Check if this video path belongs to any output index in this row
            try:
                video_map = dict(item.data(Qt.ItemDataRole.UserRole + 1) or {})
            except Exception:
                video_map = {}
            
            target_idx = -1
            for idx, path in video_map.items():
                try:
                    p_norm = os.path.normcase(os.path.normpath(str(path or "")))
                    if p_norm == src_norm:
                        target_idx = int(idx)
                        break
                except Exception:
            ...
            # Fallback check for the currently selected path
            if target_idx < 0:
                try:
                    ...
                    if selected_path:
                        ...
                        if selected_norm == src_norm:
                            target_idx = int(item.data(Qt.ItemDataRole.UserRole + 2) or 1)
                except Exception:
                    pass
            ...
            # Update the preview map for the specific output index
            try:
                preview_map = dict(item.data(Qt.ItemDataRole.UserRole + 4) or {})
            except Exception:
                preview_map = {}
            
            preview_map[target_idx] = thumb
            item.setData(Qt.ItemDataRole.UserRole + 4, preview_map)

            # If the output being updated is the one currently selected, refresh the display
            try:
                current_selected_idx = int(item.data(Qt.ItemDataRole.UserRole + 2) or 1)
                if target_idx == current_selected_idx:
                    self._render_image_preview(int(row), thumb)
            except Exception:
                pass
            
            # Refresh badges to show the thumbnail state
            self._refresh_video_badges(int(row))
    ...
    def _mode_label(self, mode_key: str) -> str:
        ...
        return labels.get(key, "GROK - Tạo video từ văn bản")
    def _set_row_mode_meta(self, row: int, mode_key: str, payload: dict | None = None) -> None:
        ...
        if prompt_item is None:
            return
        key = str(mode_key or self.MODE_GROK_TEXT_TO_VIDEO).strip() or self.MODE_GROK_TEXT_TO_VIDEO
    ...
    def _row_mode_key(self, row: int) -> str:
        prompt_item = self.table.item(int(row), self.COL_PROMPT)
        if prompt_item is None:
            return self.MODE_GROK_TEXT_TO_VIDEO
        try:
            key = str(prompt_item.data(Qt.ItemDataRole.UserRole + 1) or "").strip()
        except Exception:
            key = ""
        return key or self.MODE_GROK_TEXT_TO_VIDEO
    ...
    def _load_status_snapshot(self) -> None:
        ...
        try:
            ...
            for idx, row_data in enumerate(rows):
                ...
                mode_key = str(row_data.get("mode_key") or self.MODE_GROK_TEXT_TO_VIDEO).strip() or self.MODE_GROK_TEXT_TO_VIDEO
                if mode_key not in {self.MODE_GROK_TEXT_TO_VIDEO, self.MODE_GROK_IMAGE_TO_VIDEO}:
                    continue
    ...
    def _start_rows_by_mode(self, rows: list[int]) -> None:
        ...
        for mode_key, mode_rows in grouped.items():
            ...
            for r in mode_rows:
                ...
                if mode_key == self.MODE_GROK_IMAGE_TO_VIDEO and not str(payload.get("image_link") or "").strip():
                    continue
                if mode_key == self.MODE_GROK_TEXT_TO_VIDEO and not prompt_text:
                    continue
            ...
            if valid_rows:
                ...
                queue_jobs.append({
                    ...
                    "rows": [int(r) for r in valid_rows],
                    "label": self._mode_label(str(mode_key)),
                })

        if not queue_jobs:
            QMessageBox.warning(self, "Thiếu dữ liệu", "Không có dòng GROK hợp lệ để chạy lại.")
            return
        ...
        if not started:
            ...
            self._refresh_pending_positions()
            QMessageBox.warning(self, "Không thể chạy", "Không thể khởi động lại các dòng GROK đã chọn.")

    def _start_mode_group(self, mode_key: str, rows: list[int]) -> bool:
        key = str(mode_key or "").strip() or self.MODE_GROK_TEXT_TO_VIDEO
        ...
        QMessageBox.warning(
            self,
            "Mode chưa hỗ trợ",
            f"Mode '{self._mode_label(key)}' hiện không còn hỗ trợ.",
        )
        return False
